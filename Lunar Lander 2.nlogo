...
;; Vertical graze done   horizontal graze done
;; plot velo done
turtles-own [
...
to setup
  ...
  create-turtles 1 [
    set shape "lander"
    set color red
    set size 8       ;; lander size
    setxy (platform + max-pxcor)     ;; set x
          (max-pycor - size / 2)     ;; set y
    set heading 0    ;; head direction
  ]
  reset-ticks
end
...
to setup-terrain
  ...
  set platform platform-width                 ;; platform landing
               + random (world-width - 2 * platform-width)
               + min-pxcor
  ;; first use a turtle to draw the surface of the moon
  ;; including the landing platform
  create-turtles 1 [
    set color gray                          ;; hill top color
    setxy floor min-pxcor                         ;; minimum x-coordinate for patches, which determines the size of the world.
          floor (min-pycor / 2)
    set heading 90                        ;;  terain direction
    repeat world-width [
      set pcolor color
      fd 1
      ...
      if color != blue [
        ...
        ;; larger ones
        let y ( ycor + one-of [1 -1] * random-poisson ( terrain-bumpiness ) / 2 )
...
to autopilot
  ...
  ask turtles [
    ...
    [ setxy (xcor + xvel) (ycor + yvel) ]
    set yvel yvel + 0.001
    ;;if (abs yvel = 0) [set yvel 0.05]
    set cons -1
    if ( ( pxcor - platform ) > platform-width / 4 ) [ set xvel  -0.1  set cons 0 ]
    if ( ( pxcor - platform ) < platform-width / 4 ) [ set xvel 0.1  set cons 0 ]
    if ( cons = 0 ) [ set yvel -0.03 ]
    ;; if (abs xvel = 0) [set xvel 0.03]
    ;; if ( abs (pxcor - platform ) > platform-width / 2 )[  set xvel 0.03 if( ( pycor - platform ) <   max-pycor / 8 )  [ set yvel 0.01 ]  ]
...
to thrust  ;; turtle procedure
  if shape = "lander" [
    set xvel xvel + thrust-amount * dx      ;;dx is simply the sine of the turtle's heading
    set yvel yvel + thrust-amount * dy      ;;dy is simply the cosine of the turtle's heading
...
"" ""
PENS
"pen-1" 1.0 0 -2674135 true "" "plotxy xvelo yvelo"
...
"" ""
PENS
"default" 1.0 2 -2139308 true "" "plot-pen-reset \nplotxy xcord ycord"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plotxy xcord ycord"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot xvelo"
